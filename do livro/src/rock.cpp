...
Rock Rock::SSMM(Rock festival, int ini, int meio, int fim)
{
    ...
    {
        sum += festival.notas[i];  // Soma o valor da posição i com a soma
    ...
    {
        sum += festival.notas[j];  // Soma o valor da posição i com a soma
...
Rock Rock::SSM(Rock festival, int ini, int fim)
{
    ...
    cruzado = SSMM(festival, ini, meio, fim);
